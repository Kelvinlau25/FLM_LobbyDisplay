...
public class Upper2ndScreenDtlModel : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        if (Action == 3) // Edit
        {
            MainTitle = "Master Maintenance Second Screen Upper Video - TV 2 - Edit";
        ...
        {
            MainTitle = "Master Maintenance Second Screen Upper Video - TV 2 - Add";
        }
        return Page();
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        try
        {
            ...
            if (Action == 1) // Add
            {
                ...
                var savePath = Path.Combine(_env.WebRootPath, "acc", "LobbyDisplay2", "secscrtop", name);
                Directory.CreateDirectory(Path.GetDirectoryName(savePath)!);
                await using var stream = new FileStream(savePath, FileMode.Create);
                await FileUpload.CopyToAsync(stream);

                var sql = $"INSERT INTO MM_VIDEOS (ATTACH_FILE,SEEK_START,SEEK_END,PERIOD_START,PERIOD_END,SCR_ID,RECORD_TYP,CREATED_BY,CREATED_DATE,CREATED_LOC,UPDATED_BY,UPDATED_DATE,UPDATED_LOC) VALUES ('{name}','{SeekStart}','{SeekEnd}','{startTime}','{endTime}','8','1','{user}',GETDATE(),'{loc}','{user}',GETDATE(),'{loc}')";
