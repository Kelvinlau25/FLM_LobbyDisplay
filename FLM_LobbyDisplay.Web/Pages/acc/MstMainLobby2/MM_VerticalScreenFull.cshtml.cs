...
public class MMVerticalScreenFullModel : PageModel
{
    ...
    {
        Grid1Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=7 ORDER BY ID_MM_VIDEOS DESC");
        Grid2Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=8 ORDER BY ID_MM_VIDEOS DESC");
        Grid3Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=9 ORDER BY ID_MM_VIDEOS DESC");
    }
    ...
    private void LoadScrollingText()
    {
        try
        {
            var path = Path.Combine(_env.ContentRootPath, "acc", "MstMainLobby2", "scrollingtext.txt");
