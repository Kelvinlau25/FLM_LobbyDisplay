...
public class MMVerticalScreenFullModel : PageModel
{
    ...
    {
        Grid1Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=1 ORDER BY ID_MM_VIDEOS DESC");
        Grid2Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=2 ORDER BY ID_MM_VIDEOS DESC");
        Grid3Rows = await QueryRowsAsync("SELECT * FROM MM_VIDEOS WHERE RECORD_TYP<>5 AND SCR_ID=3 ORDER BY ID_MM_VIDEOS DESC");
    }
    ...
    private void LoadScrollingText()
    {
        try
        {
            var path = Path.Combine(_env.ContentRootPath, "acc", "MstMain", "scrollingtext.txt");
    ...
    public async Task<IActionResult> OnPostDeleteAsync(int id, int scrId)
    {
        ...
        {
            TempData["Alert"] = "Unable to Delete - Video are being used in Lobby Screen";
