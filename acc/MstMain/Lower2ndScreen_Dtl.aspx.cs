...
public partial class acc_MstMain_Lower2ndScreen_Dtl : Control.Base
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (base.Action == EnumAction.Add)
            {
                if (fileupload.HasFile)
                {
                    ...
                    if (ext.Equals(".mp4"))
                    {
                        ...
                        {
                            string paths = Server.MapPath("~/acc/LobbyDisplay/secscrbtm/" + name);
            ...
            if (base.Action == EnumAction.Add)
            {
                ...
                if (dt.Rows.Count == 0)
                {
                    seek_start = (!(txt_skstart.Text == string.Empty)) ? txt_skstart.Text : "0";
                    seek_end = (!(txt_skstop.Text == string.Empty)) ? txt_skstop.Text : "0";
                    ...
                    Sql = "INSERT INTO MM_VIDEOS (ATTACH_FILE,SEEK_START,SEEK_END,PERIOD_START,PERIOD_END,SCR_ID,RECORD_TYP,CREATED_BY,CREATED_DATE,CREATED_LOC,UPDATED_BY,UPDATED_DATE,UPDATED_LOC) VALUES ('"
                        + name + "','" + seek_start + "','" + seek_end + "','" + start_time + "','" + end_time + "','3','1','" + user + "',GETDATE() ,'" + loc + "','" + user + "',GETDATE() ,'" + loc + "')";
